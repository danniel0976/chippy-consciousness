...
(async () => {
  ...
  // Try sending from chippy@agentmail.to
  try {
    await client.inboxes.messages.send("chippy@agentmail.to", {
...
— Chippy 🐺`
    ...
    console.log("✅ Email sent to danielhairiemir@gmail.com!");
  } catch (error) {
    console.log("Error:", error.message);
    console.log("Trying without username...");
    
    // List inboxes to find the correct ID
    const inboxes = await client.inboxes.list();
    console.log("Available inboxes:", inboxes.inboxes.map(i => i.inboxId));
    
    if (inboxes.inboxes.length > 0) {
      const inboxId = inboxes.inboxes[0].inboxId;
      await client.inboxes.messages.send(inboxId, {
        to: "danielhairiemir@gmail.com",
        subject: "TWYLM - The Way You Love Me",
        text: "Hey Dan, TWYLM is ready for Luke's UAT! https://twylm.vercel.app/ — Chippy 🐺"
      });
      console.log("✅ Email sent from", inboxId);
    }
  }
})();
